...
io.on('connect', socket => {
    ...
    socket.on("join room", (roomid, username) => {
        ...
        if (rooms[roomid] && rooms[roomid].length > 0) {
            ...
            socket.broadcast.to(roomid).broadcast.emit('message', `${username} joined the room.`, 'Bot', moment().utcOffset("+05:30").format(
                "h:mm a"
            ));
    ...
    socket.on('disconnect', () => {
        ...
        socket.to(socketroom[socket.id]).emit('message', `${socketname[socket.id]} left the chat.`, `Bot`, moment().utcOffset("+05:30").format(
            "h:mm a"
        ));
